import {
  ...
  startDraggingFromWindowFullscreen,
    ...
    const handleMouseDown = async (e: MouseEvent) => {
      ...
      if (e.detail === 2) {
        try {
          ...
          if (isFullscreen) {
            await exitWindowFullscreen(appWindow);
          } else {
      ...
      if (isFullscreen) {
        try {
          await startDraggingFromWindowFullscreen(appWindow, e.clientX, e.clientY, headerRect);
    ...
    const handlePointerMove = async (e: PointerEvent) => {
      ...
      if (deltaX > 5 || deltaY > 5) {
        ...
        try {
          const isFullscreen = await appWindow.isFullscreen();
          if (isFullscreen) {
            await startDraggingFromWindowFullscreen(
              appWindow,
              e.clientX,
              e.clientY,
